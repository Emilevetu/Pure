...
const AutocompleteDark: React.FC<AutocompleteDarkProps> = ({
  value,
  onChange,
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <div className="relative">
        ...
        {/* Search icon */}
        <div className="absolute left-0 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none">
          <Search size={18} />
        </div>
        ...
        {inputValue && (
          <button
            onClick={clearInput}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-300 transition-colors p-1 rounded-full"
            type="button"
          >
            ×
          </button>
      ...
      {isOpen && filteredOptions.length > 0 && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 rounded-xl shadow-2xl border border-gray-700 max-h-64 overflow-y-auto z-50">
          {filteredOptions.map((option, index) => (
            <button
              key={option}
              onClick={() => handleOptionSelect(option)}
              className={cn(
                "w-full px-4 py-3 text-left text-sm hover:bg-gray-800 transition-colors",
                "flex items-center space-x-3 text-white",
                highlightedIndex === index && "bg-gray-800 text-white"
              )}
              type="button"
            >
