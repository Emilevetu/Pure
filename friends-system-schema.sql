-- =====================================================
-- SYSTÈME D'AMIS POUR ASTROGUIDE
-- =====================================================
...
BEGIN
  ...
  SELECT 
    ...
    END as friend_id,
    CASE 
      WHEN f.requester_id = user_id THEN u2.email::TEXT
      ELSE u1.email::TEXT
    END as friend_email,
    CASE 
      WHEN f.requester_id = user_id THEN COALESCE(u2.name, u2.email)::TEXT
      ELSE COALESCE(u1.name, u1.email)::TEXT
    END as friend_name,
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(u.name, u.email)::TEXT as requester_name,
    f.id as friendship_id,
    f.created_at
  FROM public.friends f
...
BEGIN
  ...
  SELECT 
    f.addressee_id,
    u.email::TEXT as addressee_email,
    COALESCE(u.name, u.email)::TEXT as addressee_name,
    f.id as friendship_id,
    f.created_at
  FROM public.friends f
...
DO $$
BEGIN
  RAISE NOTICE '✅ Système d''amis créé avec succès !';
  RAISE NOTICE '📊 Tables créées : friends';
